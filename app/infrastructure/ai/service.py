...
class AIServiceImpl(AIService):
    """
    AI service with automatic fallback chain:
    ...
    async def generate(
        ...
        for provider in self.providers:
            ...
            try:
                ...
                cost = self.estimate_cost(tokens_used, provider.model)
                duration = time.time() - start_time

                logger.info(f"AI generation completed ({provider.name}) in {duration:.2f}s, model={provider.model}, tokens={tokens_used}")
